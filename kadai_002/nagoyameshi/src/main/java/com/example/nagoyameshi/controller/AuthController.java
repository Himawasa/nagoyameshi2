...
public class AuthController {
	...
	private final ResetEventPublisher resetEventPublisher; // パスワードリセットイベントのパブリッシャー
	...
	public String resetMail(
			@ModelAttribute @Valid ResetForm form, BindingResult bindingResult,
			RedirectAttributes redirectAttributes, HttpServletRequest httpServletRequest) {
		// 存在しないメールアドレスのバリデーション
		if (!bindingResult.hasErrors() && !userService.isEmailRegistered(form.getEmail())) {
			FieldError fieldError = new FieldError(bindingResult.getObjectName(), "email", "メールアドレスが存在しません。");
			bindingResult.addError(fieldError);
		}
		...
		String requestUrl = new String(httpServletRequest.getRequestURL());
		resetEventPublisher.publishResetEvent(form.getEmail(), requestUrl);
		redirectAttributes.addFlashAttribute("successMessage", "メールに記載されたパスワードを確認してください。");
		return "redirect:/";
	}
